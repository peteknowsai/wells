...
Sprites are great. But the upstream company has to stay solvent for the fleet to keep working, and the substrate is locked to one Linux flavor. Wells:
...
- **Sprites-shaped surface.** Anywhere you call `sprite ...` today, `well ...` does the same thing locally. The REST shapes match too — point `SPRITES_API_URL` at the local daemon and existing tooling keeps working.
- **Future: multi-OS guests.** Once the Linux substrate is solid: macOS, Windows, Android via the same lume / Virtualization.framework path. Sprites is Linux-only — wells doesn't have to be.
...
- `welld` — Bun/TS daemon on `:7878`, sprites-shaped REST. Single owner of state.
...
Coming with Phase 0 — see [`docs/MVP-PLAN.md`](docs/MVP-PLAN.md). The build is happening incrementally via the `/mvp-wells` autonomous loop.
