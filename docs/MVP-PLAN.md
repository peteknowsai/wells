...
- [x] `engine/vwell-src/` cloned at a pinned commit; record the commit + license in `engine/vwell-src.txt`
- [x] `scripts/build-lume.sh` builds the lume Swift binary into `bin/lume`
- [x] Smoke test: `bin/lume --version` works on macOS arm64
- [x] Wrapper module `engine/vwell.ts` — typed methods for `start`, `stop`, `clone`, `delete`, `list`, `info`, `pull`
...
- [x] **A.1.3 Lifecycle states — alive vs hibernating.** Originally framed as cold/warm/hot tiering; **collapsed 2026-05-07** to two states (alive, hibernating) plus a future Frozen tier (R2 offload). See [`docs/lifecycle.md`](lifecycle.md) for the canonical model; [`docs/state-tiers.md`](state-tiers.md) keeps the original three-tier investigation as archaeology. All seven sub-boxes shipped 2026-05-12 (A.1.3.a–g). Tier transitions benchmarked in state-tiers.md § Benchmarks; scenario coverage verdict in [`findings-scenario-coverage.md`](findings-scenario-coverage.md). Phase A formally complete; remaining 1.0 wells-side work is in Phase 3 of road-to-wells-1.0 (pool-code deletion + tag).
  ...
  **Open questions A.1.3 must answer (with experiments):**
  ...
  - [x] A.1.3.c — Benchmark cold/warm/hot transitions. Numbers landed in `docs/state-tiers.md` § Benchmarks (2026-05-12): warm hibernate p95 201ms / wake p95 829ms / ssh-after-wake p95 1147ms (W.10 wake-stress, 30 cycles); cold create-from-image p95 17.4s (125 samples); hot pause 2ms / resume ~6.5s (single sample, never wired as watchdog tier); hibernate.bin ~28% of allocated RAM (280MB for 1GB well, measured across live pool eggs). Thaw primitive bonus: ~481ms per concurrent VM from one hibernated bundle. Original "Open questions" Q2/Q3/Q4 closed; the 300-cell sizing table reflects the much-smaller-than-feared disk pressure.
  ...
  - [x] A.1.3.e.2 — Hibernation lume patch shipped. `engine/vwell.ts:saveState`/`restoreState` (lines 121–135) wrap `VZVirtualMachine.saveMachineState(to:)` / `restoreMachineState(from:)`. End-to-end alive in production: pool members hibernate to `hibernate.bin` on every fill cycle, adoption restores from it sub-1s. `wells-stable-2026-05-09j` ships hibernate-backed pool to cells team; A.1.4 acceptance smokes (3/3 cycles) confirm the round trip.
  ...
  - [x] A.1.3.f — Wire tiers into welld. All sub-boxes shipped: f.1 (signing), f.2 (lume serve HTTP `/run`), f.3a (activity probe → watchdog), f.3b (watchdog hibernate path), f.4 (pause-on-idle, since superseded by hibernate-on-idle). Welld's autosleep tick (`daemon/welld.ts:1629–1670`) calls `transitionWell(name, "hibernate", …)` for every well past its idle threshold; comment in welld.ts:1641 — "Normal cells sleep should mean 'hibernate this agent,' not 'stop ...'" — captures the contract. **Signing unblock landed 2026-05-07.** `bin/lume.app` Developer-ID signed + notarized + stapled with the virtualization entitlement.
    ...
    - [x] A.1.3.f.3b — Watchdog hibernate path shipped. Welld's autosleep tick (`daemon/welld.ts:1629-1670`) hibernates every well past its idle threshold via `transitionWell(name, "hibernate", …)` — same contract as B.0.7's "normal cells sleep means hibernate, not stop." `/v1/wells/{n}/hibernate` and `/wake` endpoints live (`daemon/welld.ts:441–444`). `well info` reflects state. Memory-pressure trigger and `--fresh` wake flag deferred — the time-based trigger is what cells team uses today and it's working end-to-end (smoke-warm-pool.ts asserts the watchdog hibernates after `auto_sleep_seconds` elapses).
  - [x] A.1.3.g — Smoke: scenario coverage. Full S1–S10 verdict + live-smoke captured in [`docs/findings-scenario-coverage.md`](findings-scenario-coverage.md) (2026-05-12). Wired signal set (sig-2/3/4/5 + sig-6) covers 6/10 scenarios cleanly; two ❌ documented gaps (S1 silent compile, S6 cron pre-execution) with `auto_sleep_seconds=null` mitigation; one ⚠️ partial (S4 outbound-only daemon). Live smoke against dev welld confirmed watchdog fires on schedule + W.20 backoff catches stuck wells. Surfaced design artifact: `GET /v1/wells/<name>` touches the watchdog, so scripted observability cannot be silent (intentional, documented for cells team). Smoke at `scripts/smoke-scenario-coverage.ts`.
- [x] **A.1.4 Pre-baked pool.** All sub-items shipped (a/b/b.ii/c.i/c.ii/c.iii/c.iv/d/e/f). Welld keeps `pool_size` of pre-warmed wells in `~/.wells/pool/` (separate namespace). `well create <name>` adopts a pool member: symlinks the welld bundle (lume bundle stays at pool-XXXX since VZ saved-state encodes absolute paths), captures restore recipe, registers with `lume_name`, wakes from `hibernate.bin`, hot-swaps in-guest identity (hostname + machine-id) via SSH. Pool refills async via `triggerFillIfNeeded` after adoption + housekeeping timer. **Final live-verified end-to-end on dev: pool-adopted create 2.0-2.2s, post-hibernate wake 989ms, ssh-after-wake 1.2s** — full create-idle-hibernate-wake cycle clean. Original ≤2s create target is met by pure adoption (sub-1s); +1.4s for identity reset is the tradeoff for in-guest hostname/machine-id uniqueness vs the warm-restart alternative which would have added 10-15s. A.1.4.g (adoption restore intermittent flake) remains `~` monitored — could not deterministically reproduce after multiple stress runs.
  ...
  - [x] **A.1.4.b.ii — Welld background fill loop.** New `lib/poolFiller.ts` exports `startPoolFiller()` (spawns the housekeeping timer + fires one immediate tick on welld startup) and `triggerFillIfNeeded()` (called by `adoptFromPool` right after `removePoolMember` so the next fill kicks off without waiting for the next housekeeping tick). Concurrency=1 via a module-scope `filling` boolean — multiple fill triggers within ~12s coalesce to one hatch (avoids host memory/IO thrash from parallel `fillPoolMember` calls). Pure decision helper `shouldFill(poolSize, readyCount, inflight)` factored out for testability: `pool_size <= 0` disables, inflight blocks, fill iff `readyCount < poolSize`. Defaults gain `pool_size: number` (default `0` = disabled; cells team opts in via `defaults.json`). Welld startup wires `startPoolFiller()` and shutdown calls the returned stop function. Dev welld restarted clean with `pool_size=0`: no hatch, no errors, healthz green. 480 tests green (+7 — `shouldFill` matrix + `pool_size` defaults round-trip + missing-key fallback). Live-end-to-end verification deferred to A.1.4.e adoption smoke (which exercises trigger → fill → ready cycle properly).
  - [x] **A.1.4.c.i — Pool adoption skeleton.** New `lib/adoptFromPool.ts` exports `adoptFromPool({name, auth?})`: validates operator name, atomically reserves a ready pool member (`reserveReadyMember`, transitions to `adopting`), renames bundle dirs (`PATHS.poolMemberDir(member.name)` → `PATHS.vmDir(opts.name)`, `~/.lume/<member>/` → `~/.lume/<opts.name>/`; lume keys bundles by directory name only so `mv` is the canonical rename), captures the restore recipe from the renamed lume bundle, writes runtime.json with `state: hibernating` + `hibernate_ready: true` + `birth_media_detached_at: member.ready_at`, adds the wells registry entry, calls `wakeWell` to restore from `hibernate.bin`, then removes the pool member entry. New `PoolEmptyError` exposed for callers. Identity reset (so the in-guest hostname matches the operator's name) is deferred to A.1.4.c.ii — adopted well keeps the pool member's `pool-XXXXXXXX` hostname, machine-id, and SSH host keys. That's invisible to URL-based callers (cells via WELL_PUBLIC_BASE) but a UX wart for `well exec`. New `scripts/exp-pool-adopt.ts` driver. **Live-verification BLOCKED:** the wake step exposed a pre-existing architectural quirk — `engine/vwell.ts:LumeClient` defaults to `http://127.0.0.1:7777` (stable's lume) regardless of `WELL_LUME_PORT`, AND `killAndRestartLumeServe` (called by `wakeWell`) pkills every lume serve on the host. So a direct-from-script wake races stable welld's supervisor for control of port 7777 and times out reaching the wrong lume. Both issues are wells-side fix, separate fire (A.1.4.c.iii — `LumeClient` honors env, `killAndRestartLumeServe` scopes to its own owned PID).
  - [x] **A.1.4.c.ii — Identity reset on adoption.** New `lib/identityReset.ts` exports `resetWellIdentity({name, ip, sshKeyPath})` — opens one SSH session, runs `hostnamectl set-hostname <op-name>` + `/etc/hostname` overwrite + `rm /etc/machine-id /var/lib/dbus/machine-id` + `systemd-machine-id-setup`. SSH hot-swap chosen over the cells-team-preferred warm-restart because the latter would have added 10-15s to adoption (stop + restart + boot + identity reapply); SSH hot-swap adds ~1.2-1.5s (one SSH roundtrip). Sync (in adoption critical path) so the well is coherent on return — async would require state-tracking + race-handling not worth the sub-second savings. **Deliberate scope cut**: SSH host keys NOT regenerated. Pool members already have unique host keys per-pool-member (each ran `ssh-keygen -A` in well-firstboot.sh once); since adopted wells have unique pool members 1:1, host keys are already unique-per-well. Wells's host-side SSH wrappers all use `StrictHostKeyChecking=no` + `UserKnownHostsFile=/dev/null`, so fingerprint-mismatch warnings don't surface — reopen if cells team needs cryptographic identity rotation. authorized_keys also not changed (pool members were hatched with the host pubkey via `fillPoolMember`). Smoke gate updated: pool-adopted create now targets ≤3s (was ≤2s); identity reset is the +1.4s. Live-verified: full lifecycle smoke passes 3/3 cycles, adoption_ms=2089/2225 with 1234/1363ms identity reset within. 487 tests green.
  - [x] **A.1.4.c.iii — Lume client port-honoring + targeted kill.** `engine/vwell.ts:LumeClient`'s `DEFAULT_BASE` now reads `WELL_LUME_HOST`/`WELL_LUME_PORT` env vars (was hardcoded `http://127.0.0.1:7777` — meant any LumeClient constructed with no explicit base URL on dev welld was actually driving stable's lume; nobody noticed because the shared `~/.lume/` makes both lumes see the same bundles). `engine/lumeProcess.ts` refactored: `supervisedProcess: Subprocess | null` + `respawnInProgress: boolean` lifted to module scope so `killAndRestartLumeServe` and the supervisor coordinate. `killAndRestartLumeServe` now `process.kill(supervisedProcess.pid, "SIGKILL")` instead of `pkill -KILL -f "lume serve"` (which crossed dev/stable boundaries — surfaced in A.1.4.c.i live test where the wake step pkilled stable's lume). Falls back to old pkill behavior when `supervisedProcess` is null (lume started externally, no welld supervisor). The `respawnInProgress` gate prevents the supervisor from racing the kill+restart and double-spawning. 466 tests green.
  - [x] **A.1.4.d — Wire createWell to attempt adoption first.** `createWell()` in `lib/createWell.ts` now attempts pool adoption before the fresh-create path. Eligibility gate: `opts.env === undefined` (env requires a fresh boot to apply via cidata; pool members are past first boot). On the eligible path, `adoptFromPool` is called with a shape gate `{source_image, cpu, memory, disk_size}`; pool members whose shape doesn't match are not popped and the call surfaces `PoolEmptyError`. createWell catches `PoolEmptyError` and falls through to the legacy fresh-create path. R2 config (registry-only metadata) is forwarded via the new `r2` option on `adoptFromPool`. The `reserveReadyMember` atomic pop in `lib/poolRegistry.ts` gained a `PoolMemberCriteria` filter that lives next to the existing state filter so the "transition to adopting" stays atomic with shape matching. New unit tests (6) cover criteria mismatch (per-field rejection), partial-criteria semantics (omitted = don't care), all-match acceptance (with state transition), and shape-targeted picks among multiple ready members of differing sizes. 473 tests green.
  - [x] **A.1.4.e — Adoption smoke test.** Smoke artifact shipped (`scripts/smoke-pool-adopt.ts`). **3/3 pool-served cycles pass at <1s each** (live-verified 2026-05-09: 991ms / 886ms / 830ms with `--prefill=3 --total=3`). Two back-to-back stress runs through cold welld+lume restart (2/2 + 2/2 sub-1s pool adoptions). Cold-fallback (3rd cycle when pool drains, only on `--prefill < --total`) is gated on A.1.4.f's MAC-aware lease filter — initial regex bug (matched `01,` not Apple's actual `1,` form) caught + fixed in the same fire (487 tests green).
  - [x] **A.1.4.c.iv — Decouple lume bundle name + hibernate.bin path from operator name.** Apple's VZ saved-state encodes absolute paths to BOTH (a) the lume bundle's nvram.bin + disk.img and (b) the hibernate.bin file itself. Renaming or moving any of them invalidates restore (lume bundle rename → "config drifted: bootLoader path differs"; hibernate.bin move → "permission denied"). Fix: `WellRecord` gains `lume_name?: string` (set to `pool-XXXXXXXX` for adopted wells; falls back to `name` for fresh-create). `runtime.json` gains a `hibernate_path` that's honored by `hibernateWell` + `wakeWell` — adopted wells stick with `~/.wells-dev/pool/pool-XXXX/hibernate.bin` for both save and restore. `adoptFromPool` swaps the move-bundle-dir for `symlink(~/.wells-dev/pool/pool-XXXX/, ~/.wells-dev/vms/<op-name>/)` so code that reads via PATHS.vmDir(op-name) follows the link transparently. New `resolveLumeName(name)` + `lumeNameOf(record)` helpers in `lib/registry.ts`; all LumeClient call sites in `lib/lifecycle.ts`, `lib/wake.ts`, `lib/destroy.ts`, `lib/checkpoints.ts`, `lib/imageStore.ts`, `lib/createWell.ts:diskUsageBytes`, and `daemon/welld.ts` (3 sites) route through them. Backward compat: lume_name absent → fall through to name, hibernate_path absent → PATHS.vmHibernate(name). DHCP/SSH/proxy unaffected (IP-keyed). 5 new resolveLumeName tests + the existing 480 stay green (485 total). **Live-verified 2026-05-09 02:27 PT**: 2 back-to-back pool adoptions in 811ms + 820ms.
  - [x] **A.1.4.f — Stale-lease bug in cold-fallback after pool consume.** Root cause: `findNewLeases` returned any lease entry whose `(ip, lease)` pair wasn't in the pre-boot snapshot — but vmnet's lease RENEWAL of an adjacent VM (e.g., the just-adopted pool member's wake-time refresh) creates a new entry with the SAME ip and a different lease epoch, which `findNewLeases` returned as "new". Fresh-create's `waitForDhcpLease` then handed back the unrelated VM's IP within 16ms instead of waiting for the actual new boot's lease. Fix in `lib/dhcp.ts`: `LeaseSnapshot` gains `mac: string | null` (extracted when `hw_address=01,<mac>`); `lib/createWell.ts:waitForDhcpLease` filters delta candidates by MAC when the new VM's MAC is known, falling through to the unfiltered list only when no entry in the snapshot carries a MAC at all (preserves compat for pre-A.1.4.f wells without `dhcp-identifier:mac`). 1 new test for MAC extraction in `parseAllDhcpLeases`. 486 tests green. Live verification of cold-fallback was blocked by a separate intermittent failure on the adoption restore path — surfaced as A.1.4.g.
  - [~] **A.1.4.g — Adoption restore intermittent "permission denied".** Could not deterministically reproduce after the A.1.4.f regex fix landed: 5+ back-to-back adoptions across 2 stress runs (cold welld+lume restart, hatched-then-immediately-adopted) all succeeded at 813-991ms. Original failure observed 2026-05-09 02:46 PT but didn't recur after restart-and-rerun. Could have been a one-off VZ kernel-state quirk or interaction with the regex bug surfacing as a follow-on race. Leaving the checkbox as `~` (monitored) — reopen if it surfaces again. Mitigation candidates remain documented for fast-fix if needed: (a) settle-delay after killAndRestartLumeServe, (b) skip the kill when adopting from a freshly-hatched member, (c) wait-for-fsync before restart.
- [x] **A.1.5 Pool config.** `defaults.pool_size` was added in A.1.4.b.ii (default 0 = pool disabled; cells team raises it to opt in). New REST routes in `daemon/welld.ts`: `GET /v1/wells/pool` returns `{members, target_size, ready_count}` (placed before the `/v1/wells/:name` catch so `pool` doesn't dispatch to `handleGetWell`); `POST /v1/wells/pool/refill` calls `triggerFillIfNeeded` (idempotent — no-op if depth at target or fill in flight); `POST /v1/wells/pool/drain` invokes `drainReadyPoolMembers` which lume.deletes each `state=ready` member, rms its welld pool dir, and removes the registry entry (in-flight members left alone — caller's job to set `pool_size=0` first if they want zero depth). New TypeBox shapes `PoolMemberResource`, `PoolListResponse`, `PoolActionResponse` in `lib/schemas.ts` enforce response contracts. CLI gains `well pool (list | refill | drain)` in `cli/well.ts` with `--json` on list. Live-verified end-to-end on dev: `pool list` empty → set `pool_size=1` → `pool refill` → member transitions provisioning→warming→ready in 11s → `pool drain` removes it. 487 tests green.
- [x] **A.1.6 Smoke: full lifecycle.** `scripts/smoke-warm-pool.ts` shipped — refills pool to depth=1, pool-adopts a well, sets `auto_sleep_seconds=5`, polls runtime.json until watchdog hibernates (≤60s wall, watchdog ticks every 30s), wakes via `POST /v1/wells/<n>/wake`, SSH-probes the new IP. Asserts ≤2s create, ≤2s wake, ≤5s ssh-after-wake. Restores `defaults.pool_size` on cleanup so successive runs don't pile up. **Live-verified end-to-end on dev:** create 863ms, wake 989ms, ssh 1228ms — all sub-2s, well under target. First run surfaced two A.1.4.c.iv plumbing gaps: (a) the watchdog's `runningNames` set was keyed by `record.name`, missing adopted wells whose lume bundle is `pool-XXXX`; (b) `addWell` in `adoptFromPool` didn't carry over the lume bundle's `mac_address`, so `resolveWellIp` returned null because the in-guest hostname is still `pool-XXXX`. Both fixed in the same fire (`readLumeMac` exported from `lib/createWell.ts`; watchdog resolves through `lumeNameOf`). 487 tests green.
...
- [x] **B.0.9.c — Live-verify hibernate/wake end-to-end against a fresh fork.** Absorbed by B.0.9.d.4.e (3/3 cycles `WAKE WORKS`) + W.10 wake-stress smoke 30/30 post-reboot (2026-05-10). Wake p95 829ms, ssh-after-wake p95 1147ms.
...
- [x] **B.0.9.d.2 — Disk-only steady-state hibernation.** Absorbed by B.0.9.d.4 — the warming sequence in `lib/createWell.ts:532-631` boots-with-cidata → SSH-halts → restarts-without-mount → flips `hibernate_ready: true`. B.0.9.d.4.e's "WAKE WORKS" 3/3 note explicitly credits this work: "the earlier B.0.9.d.2 disk-only steady-state work was load-bearing — without it, restore fails on cidata storage device shape mismatch."
...
- [x] **B.0.11.c — Lume client fetch timeout.** `engine/vwell.ts`'s private `request` method now uses `AbortController.signal` with a per-call `timeoutMs` (default 35s, matches the supervisor respawn budget). `saveState`/`restoreState` override to 120s for hibernate/wake on big cells. Aborted requests throw a clear "timed out after Nms — lume serve likely hung mid-request" instead of hanging welld forever. Test: `engine/lume.test.ts` adds a hung-server case that asserts a 100ms timeout aborts in <2s. 432 tests green.
...
- [x] **B.0.11.d — Investigate concurrent fork lume crash.** Resolved by W.13 (2026-05-10): `scripts/exp-concurrent-fork.ts --range=2,3,4,5,6` proved lume itself is stable at all tested N — the actual ceiling is vmnet's DHCP racing. N=4 all succeed cleanly (~12s each); N=5 hits 1 DHCP timeout, N=6 hits 2 timeouts. Failure mode is parallel-bootp-race in vmnet's bootpd, not lume/VZ. Findings: `docs/findings-concurrent-fork-crash.md`. Cells team can fan-out up to 4 concurrent fresh-creates safely; ≥5 needs serialization or tolerated-failure pattern.
...
- [x] **Lume SIGINT-on-destroy root cause.** Pinned to `engine/vwell-src/src/VM/VM.swift:451-454` — `kill(pid, SIGINT)` propagates back to lume serve via shared process-group semantics. Patched at `engine/vwell-src/ in-tree edit (SIGTERM instead of SIGINT)` (SIGTERM instead). **Plus a related bug surfaced**: `RunVMRequest` was missing a `mount` field, forcing wells to spawn `lume run` as a subprocess — those VMs aren't in lume serve's SharedVM cache, so pause/resume fail. Patched at `engine/vwell-src/ in-tree edit (the mount field on RunVMRequest)`. `lib/createWell.ts` migrated to use the HTTP API path (commit `1372055`). End-to-end: pause/resume work first try on freshly-created wells; lume crashes drop from ~1+/cycle to ~1/3 cycles in 10-cycle stress.
...
- [x] **Long-term: SharedVM survives lume serve restart.** Shipped via B.0.6 (orphan-XPC sweep on lume serve startup + stale-session cleanup). Disk persists; in-RAM state lost on lume restart but `well start` brings VMs back via fresh boot. See B.0.6 for the implementation.
...
- [ ] **Lume Swift patch**: add `setBalloon(targetMB)` API on the running VM. Calls Apple's `setTargetVirtualMachineMemorySize` on the existing `VZVirtioTraditionalMemoryBalloonDevice`. The device is already wired in lume's VM config (`engine/vwell-src/src/Virtualization/VMVirtualizationService.swift:305,467`); we just need to expose the control. ~50-80 lines.
- [ ] **HTTP route**: `POST /lume/vms/:name/balloon` body `{target_memory_mb: 512}`.

#### C.2 — Welld wrapper + pressure controller

- [ ] **`LumeClient.setBalloon(name, mb)`** in `engine/vwell.ts` — TypeScript wrapper for the new lume route.
...
- [ ] **Document the engine interface.** `engine/INTERFACE.md` enumerates the contract every engine satisfies: `create`, `delete`, `start`, `stop`, `info`, `list`, plus the disk path conventions and cidata expectations. `engine/vwell.ts` is reference impl.
