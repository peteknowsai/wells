...
- [ ] **A.1.4 Pre-baked pool.** Welld keeps `pool_size` (default 1) of pre-baked, pre-booted-then-warmed wells in `pool/` (separate registry namespace). `well create <name>` adopts a pool member: rename, re-cidata for identity, restore from warm state. Target: <2s end-to-end. Pool refills async after adoption. Depends on A.1.3.
  ...
  - [~] **A.1.4.e — Adoption smoke test.** Smoke artifact shipped (`scripts/smoke-pool-adopt.ts`) — pre-fills pool with N=2 (resumable: skips re-prefill if depth already met), drives 3 well-create requests through welld's HTTP API, asserts per-cycle target. **Live verification surfaced an architectural blocker** (see `docs/findings-pool-adopt-bundle-rename.md`): Apple's `VZVirtualMachineConfiguration` saved-state encodes absolute paths for boot loader (nvram.bin) and storage attachments. The current `adoptFromPool` renames the lume bundle dir on adoption (`~/.lume/pool-XXXX/` → `~/.lume/<op-name>/`), which invalidates VZ restore — `restoreState` rejects with "VZ config drifted between save and restore — bootLoader: …/pool-XXXX/nvram.bin → …/<op-name>/nvram.bin". Earlier B.0.11.g portability probe didn't catch this because it kept both bundle dirs alive (no rename). Fix scoped to A.1.4.c.iv (next sub-checkbox): keep the lume bundle name stable as `pool-XXXX`, only rename welld's bundle dir + registry record (carry a `lume_name` field). Leaves smoke unchecked until c.iv lands.
  - [ ] **A.1.4.c.iv — Decouple lume bundle name from operator name.** Apple's VZ saved-state encodes absolute paths to nvram.bin + disk attachments; renaming the lume bundle on adoption invalidates restore. Fix: `WellRecord` gains `lume_name?: string` (defaults to `name` for fresh-create wells; set to `pool-XXXXXXXX` for adopted wells). `adoptFromPool` stops renaming `~/.lume/<member>/`. All `LumeClient` calls swap `name` → `lume_name`. DHCP/SSH/proxy layers key by IP so they're unaffected. `well destroy` cleans up via `lume_name`. Backward compat: existing wells without `lume_name` fall back to `name`.
