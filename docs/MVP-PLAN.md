...
- [ ] **A.1.4 Pre-baked pool.** Welld keeps `pool_size` (default 1) of pre-baked, pre-booted-then-warmed wells in `pool/` (separate registry namespace). `well create <name>` adopts a pool member: rename, re-cidata for identity, restore from warm state. Target: <2s end-to-end. Pool refills async after adoption. Depends on A.1.3.
  ...
  - [ ] **A.1.4.f — Stale-lease bug in cold-fallback after pool consume.** When a pool member is adopted, lume's vmnet keeps the lease entry for that pool's MAC/IP in `/var/db/dhcpd_leases`. The next fresh-create gets a NEW MAC from lume — but `waitForDhcpLease` returned the consumed pool member's old IP within 16ms (snapshot didn't filter it because the lease epoch matched the pre-create snapshot). Surfaces only on the cold-fallback path; pool-served path uses the pool member's existing lease. Fix: filter delta-snapshot by MAC, not just (ip, lease) pair — fresh-create's lume bundle has a different MAC than the pool member's. Or invalidate the consumed pool's lease entry post-adopt.
