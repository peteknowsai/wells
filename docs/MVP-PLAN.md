...
- [ ] **A.1.4 Pre-baked pool.** Welld keeps `pool_size` (default 1) of pre-baked, pre-booted-then-warmed wells in `pool/` (separate registry namespace). `well create <name>` adopts a pool member: rename, re-cidata for identity, restore from warm state. Target: <2s end-to-end. Pool refills async after adoption. Depends on A.1.3.
  ...
  - [~] **A.1.4.e — Adoption smoke test.** Smoke artifact shipped (`scripts/smoke-pool-adopt.ts`). After A.1.4.c.iv, **pool adoption now lands in <1s** (live-verified 2026-05-09: 811ms + 820ms for 2 back-to-back adoptions; both well under the ≤2s target). Cold-fallback (3rd cycle when pool drains) hit a separate latent stale-lease bug — fresh creates after a pool member is consumed inherit the pool member's recycled MAC/IP and `waitForDhcpLease` returns the stale entry within 16ms. Tracked separately as A.1.4.f (cold-fallback after pool consume). Smoke artifact is good; full 3-cycle pass is gated on A.1.4.f.
  ...
  - [ ] **A.1.4.g — Adoption restore intermittent "permission denied".** `lume.restoreState` on a freshly-hatched pool member sometimes returns `"An error occurred while restoring the virtual machine. The virtual machine failed to restore with error 'permission denied'."` even though the same hibernate.bin restores cleanly via direct `POST /lume/vms/<n>/restore-state`. Reproduced 2026-05-09 02:46 PT against `pool-f81dfced` (created in same smoke run). Manual restore against `pool-187c5469` (sibling, same smoke run) immediately worked. Difference: adoption goes through `wakeWell` → `killAndRestartLumeServe` → `restoreState`; manual path skips the restart. Hypothesis: VZ kernel-level state for the hibernate.bin gets bound to the lume PID at save-time and tearing down lume immediately invalidates it. Mitigation candidates: (a) add a settle-delay after killAndRestartLumeServe before restoreState, (b) skip killAndRestartLumeServe when adopting (we're not racing a pre-existing VM cache like the wake-from-prior-hibernate case), (c) save → wait-for-fsync → restart so the file is genuinely on-disk before lume goes away. Earlier 02:27 smoke ran 2 successful adoptions in a row, so this isn't 100% — points at a race.
