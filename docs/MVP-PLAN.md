...
- [ ] **A.1.3 Cold / warm / hot tiering — core lifecycle research.** This is the most foundational work in Phase A. The current touch-on-API-call model from A.1.1 is too coarse: real activity (in-guest compile, ssh session, agent loop, background job) often doesn't hit our API, and the watchdog could kill a splite mid-work. Pete's call: don't just ship save/restore — iterate until we know the answer to the questions below. Multi-fire investigation; ships when defaults are picked from data.
  ...
  **Open questions A.1.3 must answer (with experiments):**
  ...
  - [ ] A.1.3.e.2 — Warm-tier lume patch (~300 lines, build saveState/restoreState from scratch).
  ...
  - [ ] A.1.3.f — Wire tiers into splited. **Blocked on signing — see `docs/BLOCKED.md`.** Hot-tier code is implemented end-to-end (lume patches in vendor/, `engine/lume.ts` pause/resume methods, `lib/lifecycle.ts` `pauseSplite`/`resumeSplite`) but `bin/lume` lacks the `com.apple.security.virtualization` entitlement. Pete has an Apple Developer account that can resolve this. Decomposed into:
    - [x] A.1.3.f.1 — Investigated. Original "long-poll/MainActor" theory was wrong: `handleRunVM` already uses `Task.detached` and returns 202 immediately. Real blocker found by capturing lume serve's previously-discarded stderr (`engine/lumeProcess.ts` was piping it to `"ignore"`): `Failed in VM.run … process doesn't have the "com.apple.security.virtualization" entitlement`. Verified that even an adhoc-signed `.app` bundle hits the same error — the entitlement requires Developer-ID + provisioning profile. Added stderr capture to `/tmp/lume-serve.log`. Updated `state-tiers.md` § "The entitlement gap (A.1.3.f.1)" + `BLOCKED.md`.
    - [ ] A.1.3.f.2 — Once Developer ID + provisioning profile are set up: rebuild `bin/lume` properly, switch `startSplite` to `lume serve`'s HTTP `/run`, live-test pause/resume cycle on pete.
    - [x] A.1.3.f.3a — Activity probe feeds watchdog (cold-only). `runWatchdogTick` accepts a `probeActivity(name)` callback; if it reports active, we `touch(name)` before the sleep decision. Splited wires it via `readDhcpLease` + `sampleActivity`. 12 unit tests across `lib/watchdog.test.ts`. Runs every 30s alongside the existing tick. Probe failure is non-fatal — falls back to touch-only logic.
    - [ ] A.1.3.f.3b — Tier picker (hot/warm/cold) wired into the watchdog. Blocked on entitlement signing — until `bin/lume` can pause/save VMs, "stay running vs cold" is the only meaningful split. `splite stop --cold` and `splite info` tier surface roll in here.
