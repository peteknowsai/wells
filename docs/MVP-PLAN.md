...
- [ ] **B.0.9.d.4.c — Re-bake `ubuntu-25.10-base`.** Run `scripts/bake-base-image.ts --force` to produce a base image with cloud-init stripped. Currently running in background. Verify: SSH into the staging well after bake, confirm `well-firstboot.service` exists, confirm `which cloud-init` returns nothing.
- [x] **B.0.9.d.4.d — Wire wellSeed + restore warming sequence.** `lib/createWell.ts` switched from `composeWellUserData/buildCidata` to `buildWellSeed` (well.env + authorized_keys ISO). Warming sequence restored: boot with cidata → wait `/etc/.well-ready` → stop → restart without mount → wait SSH → write `runtime.json` with `hibernate_ready: true`. `lib/lifecycle.ts` `hibernateWell` gates on `hibernate_ready === true`; `wakeWell` drops the cidata mount. `lib/wellRuntime.ts` runtime gains the seal fields, `RestoreRecipe` flips to disk-only contract. `lib/restoreRecipe.ts` `captureRestoreRecipe` always sets `mount_path: null` + `storage_device_count: 1`. 438 wells tests green. Live-verify waits on .c.
