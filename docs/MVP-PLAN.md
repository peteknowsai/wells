...
- [ ] **A.1.3 Cold / warm / hot tiering — core lifecycle research.** This is the most foundational work in Phase A. The current touch-on-API-call model from A.1.1 is too coarse: real activity (in-guest compile, ssh session, agent loop, background job) often doesn't hit our API, and the watchdog could kill a splite mid-work. Pete's call: don't just ship save/restore — iterate until we know the answer to the questions below. Multi-fire investigation; ships when defaults are picked from data.
  ...
  **Open questions A.1.3 must answer (with experiments):**
  ...
  - [ ] A.1.3.f — Wire tiers into splited. **First sub-attempt revealed an architectural gap** — see `docs/state-tiers.md` § "Lume's process model": pause/resume only work on VMs lume serve launched, but splited uses `lume run` subprocess. Adding `engine/lume.ts` `pause(name)`/`resume(name)` methods landed; `lib/lifecycle.ts` `pauseSplite`/`resumeSplite` are wired but currently no-ops because of the cache problem. Decomposed into:
    - [ ] A.1.3.f.1 — Investigate the long-poll/MainActor serialization more carefully. If lume's `/info` endpoint can be marked `nonisolated`, the fire-and-poll pattern works and we ship hot tier.
    - [ ] A.1.3.f.2 — If A.1.3.f.1 doesn't pan out, design the workaround (Swift worker per VM, or wait for Phase E's Firecracker shift which has different process semantics).
