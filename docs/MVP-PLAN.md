...
- [ ] **B.0.9.d.4.e — Live-verify hibernate/wake.** Create+warm: ✓ (15.0s, on-target). Hibernate: ✓ (189ms). Wake: ✗ Apple Virt rejects `restoreMachineStateFrom` with "Transition from state 'paused' to state 'restoring' is invalid" — known structural issue (commit 8c54a99 documented same-instance restore impossibility). The drop-handle pattern (`virtualizationService = nil` in saveState) was meant to fix this by forcing a fresh VZVirtualMachine on restore, but Apple's framework appears to keep VZ state at kernel level keyed by disk path, so the "fresh" instance still inherits the saved `.paused` state. Adding `lume.stop` before restore changes the error to "permission denied" but doesn't unblock. Needs Apple-Virt-level investigation or a different hibernation primitive. Smoke test artifact: `scripts/smoke-hibernate-wake.ts`.
