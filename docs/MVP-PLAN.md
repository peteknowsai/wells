...
- [ ] **B.0.9.d — Storage attachment drift between save and restore.** After stripping non-essential devices (B.0.9.b), Apple's restoreMachineStateFrom now rejects with "The storage device attachment is invalid" rather than generic "invalid argument". VZ config snapshots are byte-identical save/restore — drift is in the storage attachment's non-serializable internal binding (file handle? bookmark? inode?). Next moves: (1) try `VZDiskImageStorageDeviceAttachment(... cachingMode: .uncached)` since `.cached` may hold a Mach VM region; (2) try keeping the same `VZVirtualMachine` instance across save/restore (don't drop the VZ handle in `saveState`); (3) binary-search storage devices — strip cidata, restore disk-only, see which attachment Apple rejects.
