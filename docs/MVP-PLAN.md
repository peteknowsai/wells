...
- [x] **Lever 3 (b): Cell-side static netplan injection (code complete; e2e reliability pending B.0.5).** `composeWellUserData(template, keys, env, pinnedIp?)` now drops `/etc/netplan/99-wells-pinned.yaml` via `write_files` (a higher-numbered file netplan merges over `50-cloud-init.yaml`, with `dhcp4: false` taking precedence). The static template's runcmd appends `netplan apply` so the cell switches off DHCP onto its pinned IP after first-boot networking comes up. `createWell` allocates pinned IP via `nextPinnedIp()`, threads it into composeWellUserData, persists `pinned_ip` on the WellRecord, and `waitForSshReady` probes the pinned IP (with a best-effort DHCP wait first to ride the transient). **Live evidence (this fire)**: ARP showed `192.168.64.100` bound to two separate freshly-booted forks' MACs (pin-1's `72:11:32:4e:33:8b`, pin-2's `fe:50:d2:59:8d:8f`) within ~30s of `lume.start` — proves the `netplan apply` ran and brought the static IP up on the cell. Diagnostic finding: forks from `cell-base` (saved earlier this session) don't boot to network at all; forks from `ubuntu-25.10-base` do. Use the pristine base going forward; rebuild `cell-base` from a working pinned fork when needed. **End-to-end SSH-ready at `.100` not yet observed in a single run because `lume serve` crashes ~33s into pinned forks** — pre-existing instability, see B.0.5 below.

#### B.0.5 — Lume serve crashes mid-fork (NEW, surfaced 2026-05-08 during Lever 3 verification)

`lume serve` self-respawns every 30-35s during a pinned-IP fork from `ubuntu-25.10-base`. Watchdog respawns the binary, but the in-flight VM dies (`status: stopped`). Two fires now have hit the same window. A DHCP-only fork (diag-pristine) survived, so the trigger may be related to the additional cloud-init runcmd / netplan apply step or pure timing — needs a controlled bisect.

- [ ] **Repro reliably.** Ten serial forks from `ubuntu-25.10-base`, half with pinned IP, half without. Time each crash relative to lume.start. Capture `/tmp/lume-serve.log` around the crash window. Confirm whether netplan apply is the trigger or just a coincidence.
- [ ] **Patch lume serve respawn supervisor.** If the crashes have a known signature, either fix in the patched lume (Swift) or shift welld's supervisor to wait-and-retry the in-flight VM rather than letting it stay dead.
- [ ] **Smoke pinned IP end-to-end.** Once lume is stable, run a 5x sequential pinned-fork test: each well comes up at its allocated `192.168.64.100..104`, SSH responds, `pin_ip` matches `pinned_ip`. Then save a working pinned fork as the new `cell-base` for the cells team.
