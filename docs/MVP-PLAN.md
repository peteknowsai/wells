...
- [ ] **B.0.9.d.4.e — Live-verify hibernate/wake.** Create+warm: ✓ (15s on-target). Hibernate: ✓ (200ms-4s). Wake: ✗ — but **error changed** after attempted fixes. Progression so far: (1) original error "Transition from state 'paused' to state 'restoring' is invalid" — drop-handle pattern (`virtualizationService = nil`) wasn't enough because Apple Virt's VM state lives at kernel level via the VirtualMachine.xpc child process. (2) Added `killAndRestartLumeServe()` to engine/lumeProcess.ts: between hibernate and wake, SIGKILL `bin/lume serve` + spawn fresh. Lume's orphan-sweep on startup kills the VirtualMachine.xpc child; the fresh VZVirtualMachine then has truly clean kernel state. Wake error becomes "An error occurred while restoring the virtual machine. The virtual machine failed to restore with error 'invalid argument'". This is the same shape as cells team's "storage device attachment is invalid" — device-shape mismatch — except `hibernate.config.json` and `hibernate.config.restore.json` are byte-identical (only label and timestamp differ). VZ snapshot doesn't capture whatever Apple checks at restore time. Next direction: deeper into VZ device identity (NAT attachment, nvram, USB controllers) or pivot to alternative hibernation primitive. Smoke test artifact: `scripts/smoke-hibernate-wake.ts`.
