...
- [ ] **A.1.4 Pre-baked pool.** Welld keeps `pool_size` (default 1) of pre-baked, pre-booted-then-warmed wells in `pool/` (separate registry namespace). `well create <name>` adopts a pool member: rename, re-cidata for identity, restore from warm state. Target: <2s end-to-end. Pool refills async after adoption. Depends on A.1.3.
  ...
  - [ ] **A.1.4.b — Pool fill (background).** Welld background task that walks the pool toward `pool_size` ready members. Hatches a new member: clone base image → boot with generic-identity cidata → wait /etc/.well-ready → hibernate → mark ready. Runs on welld startup + after each adoption + on a slow timer.
  - [ ] **A.1.4.c — Pool adoption.** New `lib/adoptFromPool.ts`: pop ready member via `reserveReadyMember`, rename bundle (welld + lume side), reset identity (rm /etc/.well-ready, swap cidata), wake from hibernate.bin via existing wake path, write new well's registry record, remove from pool. Identity-reset details TBD — possibilities: SSH hot-swap (hostname/machine-id without restart), or warm-restart with new cidata. The cells team's proposal in `docs/proposals/cells-pool-on-wells.md` favors warm-restart.
