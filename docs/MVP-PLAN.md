...
- [ ] **Lever 3 (b): Cell-side static netplan injection** — wire `nextPinnedIp()` into `createWell` and inject a static netplan into the cell's cidata so it actually uses its pinned IP. Two attempts at integration this fire (cidata's network-config, then write_files + runcmd `netplan apply`) didn't pass live boot — both got the cell into "running" status with `networkMode: nat` but the cell never DHCP'd OR brought up the static IP. Hypothesis: stale VZ zombie process + lume serve in a degraded state. Next fire: cleanup zombies, watch cloud-init via VNC console, get the netplan injection working. Code path is mapped, just needs a clean lume environment to verify.
