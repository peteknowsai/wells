...
- [x] **Bump `MISSES_BEFORE_RESPAWN` from 6 → 24.** 30s → 2 minutes before lume is considered dead. Empirically observed VM-spawn blocking is ~30-35s; 2min is 4× margin and still acceptable as a "lume actually died" detection latency since nothing else makes progress while lume is hung anyway. Live verified: smoke-1 (fresh fork from `ubuntu-25.10-base`) went from `lume.start` to SSH-ready in **6 seconds** with no respawn — full clean boot.
