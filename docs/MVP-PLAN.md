...
- [x] **Identity-rinse implemented in welld** — `well image save --clean <well> <name>` (or `clean: true` on `POST /v1/wells/images`). Welld wakes the source, SSHes in, runs `cloud-init clean --logs --seed`, scrubs `/etc/machine-id`, `/etc/ssh/ssh_host_*`, `/var/lib/cloud/{instances,data,sem}/*`, `/var/lib/systemd/network/*`, `/var/lib/dhcp/*`, MAC-pinned netplan files (`/etc/netplan/50-cloud-init.yaml`, `00-cloud-init-fallback.yaml`), `/etc/.well-ready`, and `/etc/hostname`, stops the well, then clonefiles. `--rinse-user=USER` for legacy wells. 10 unit tests on the rinse script content.
- [x] **Rinse extended after cells team smoke** — initial rinse (machine-id + ssh keys + cloud-init `instances/` + hostname) wasn't enough; forks from rinsed images timed out waiting for DHCP. Root cause: cloud-init's MAC-pinned `/etc/netplan/50-cloud-init.yaml` and systemd-networkd's DUID/lease cache. Now using `cloud-init clean` + zapping `/var/lib/{systemd/network,dhcp}/` + the MAC-pinned netplan files. Cells team verifies on next bake.
