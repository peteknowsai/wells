...
- `PATCH /v1/sprites/{name}` body `{memory: "2GB"}` — resize a **stopped** well's RAM (2026-06-10, cells ask #4). Applies on next start; disk + identity preserved, so the flow for an existing well is stop → PATCH → start (seconds, no re-birth). "Stopped" means the **engine** sees it stopped — the runtime record may still read `alive_running` after a clean stop (it's intent, not observation; normal) and that does NOT block the resize. 409 `well_not_stopped` only while the engine reports running (VZ pins memory at boot — no live resize exists), 409 `well_hibernating` while hibernated (the saved state pins its size; wake + stop first). Same PATCH still takes `auto_sleep_seconds`; both keys in one body is fine. CLI: `well resize --memory 2GB -s <name>`. Create-time sizing has existed all along: `well create --memory 2GB` / POST body `memory` — the 1GB default (≈947MB usable in-guest) is just what you get when it's omitted.
