...
Wells today runs on Apple Virtualization.framework via vendored lume.
That's macOS-only, which means "self-hosting wells" today means
"self-hosting on a Mac in your closet" — fine for a dev rig, awful as a
production target. Pete wants wells to live on a Linux VPS too, so the
...
The question was whether to fork wells into a sibling project (working
name was "svites") for the Linux flavor, or to extend wells to support
both engines.
...
interface. Welld picks one at startup via `WELL_ENGINE` env var or
auto-detect on host OS.

Well-level concerns — registry, ssh keys, cloud-init composition, REST
...
- The Mac engine is no longer the destination. It's a dev convenience and
  a "wells also runs on the laptop you're holding" affordance. The
  prod path is Linux on a VPS, with Cloudflare Tunnel out front.

## Naming

We don't fork the brand. The Linux variant is just "wells running on
Linux." The repo stays `wells`. The product stays wells. The CLI
binary stays `well`. The only thing that changes per host is which
