...
- [ ] **W.14 — Rename `vendor/lume/` → `engine/wellv/` + drop `vendor/lume.patches/`.** Pete's pick (2026-05-10): rename + drop. Plan: (1) `git mv vendor/lume engine/wellv`; (2) `git mv engine/lume.ts engine/wellv.ts`; (3) `git rm -r vendor/lume.patches`; (4) grep for any remaining `vendor/lume` / `lume.ts` references — `scripts/build-lume.sh` (rename to `scripts/build-wellv.sh`), `engine/lumeProcess.ts` (consider renaming to `engine/wellvProcess.ts`), `engine/bundle.ts`, all `import` paths in `lib/` and `daemon/`; (5) update CLAUDE.md to remove the TODO + reflect the new layout; (6) `bun test` (must stay 493 green); (7) rebuild `bin/lume` to confirm the build script still works (or rename it `bin/wellv` if appropriate). The boundary intent stays the same — `engine/wellv.ts` is what the rest of wells imports; everything below is the swappable Mac VM engine. ~2-4 hr. Owner: `worker`. Tags: `code`, `lume-vendor`.
