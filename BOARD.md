...
> **State as of 2026-05-10 ~12:25 UTC (post-host-reboot):** **W.27 wake regression RESOLVED** by Pete's host reboot. Wake-stress smoke 30/30 cycles passed (wake p95 829ms, ssh p95 1147ms). Cells team's autosleep + wake-on-traffic flow fully operational again — `auto_sleep_seconds: null` mitigation can be dropped. Stable lives at `wells-stable-2026-05-10d`; code is unchanged, only host state was wedged. Remaining open items: **W.2 (R2 round-trip smoke)** still needs a bucket-scoped R2 token (Pete-side); **W.22 (steward starvation, durable fix)** still a Pete architectural call. **W.10 wake-stress** + **W.26 thaw end-to-end** now actually verifiable.
>
