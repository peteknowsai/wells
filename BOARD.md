...
- [x] **2026-05-14** — **Admission control + version bump + create-orphan reap.** Three fixes off the cells-pool-re-bake-timeout investigation. (1) Version string bumped `0.1.0-pre → 1.0.0` (`1eceac9`) — was never bumped at the tag cut. (2) `createWell` now reaps the VM a post-`lume.start` failure leaves behind (was leaking it + its VZ process), and `well doctor` stopped mislabeling a stable-serve orphan as a "crashed lume serve" (`f4e6daf`). (3) **Admission control** (`63a3b94`) — `lib/admission.ts` `BootGate`, a bounded-concurrency semaphore (count + vCPU backstop) wired into `create`/`start`/`wake`, so a boot burst paces itself instead of oversubscribing the host — the failure that timed out the cells re-bake. `/healthz` exposes `boot_gate`. Proposal: `docs/proposals/wells-admission-control-for-dummies.html`. Suite 993 → 1015. Goes live on the next welld bounce.
