...
```ts
pi.on("agent_end", () => fetch("http://host.splite:7879/v1/cells/me/sleep", { method: "POST" }));
```

That's it. The whole extension. From inside a splite-hosted cell, drop this into your `.pi/settings.json`:

```json
{
  "extensions": [
    "node_modules/splite-cooperate/index.ts"
  ]
}
```
...
Set `SPLITE_METADATA_URL` if your splited metadata server isn't at the default `http://host.splite:7879` (it almost always is — `host.splite` is seeded into `/etc/hosts` at first boot via cloud-init).

## What it does NOT do

- No `/working` signal. The agent doesn't need to tell splited it's busy mid-turn — by the time the watchdog might consider pausing, agent_end has already fired and the cell is off. The watchdog's outside-in 60s heuristic stays as a fallback for cells without this extension.
...
- splited's cooperation API spec: `splites/docs/cooperation.md`
