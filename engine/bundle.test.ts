...
describe("bundle paths", () => {
  ...
  beforeEach(() => {
    original = process.env.SPLITES_LUME_STORAGE;
  });

  afterEach(() => {
    if (original === undefined) delete process.env.SPLITES_LUME_STORAGE;
    else process.env.SPLITES_LUME_STORAGE = original;
  });
  ...
  test("SPLITES_LUME_STORAGE overrides root", () => {
    process.env.SPLITES_LUME_STORAGE = "/tmp/fake-lume";
